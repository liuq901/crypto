...
def work(fake_g, groundtruth):
    alice = Client(fake_g)
    bob = Client(fake_g)
    eve = Client(fake_g)
    eve.secret = groundtruth
    ...
    eve_msg = eve.decrypt(alice_to_eve, None)
    eve_to_bob = eve.encrypt(eve_msg, None)
    bob_msg = bob.decrypt(eve_to_bob, eve.public_key)
    bob_to_eve = bob.encrypt(bob_msg, eve.public_key)
    eve_msg2 = eve.decrypt(bob_to_eve, None)
    eve_to_alice = eve.encrypt(eve_msg2, None)
    alice_msg2 = alice.decrypt(eve_to_alice, eve.public_key)
    assert alice_msg == eve_msg == bob_msg == eve_msg2 == alice_msg2

def check(fake_g, groundtruth):
    try:
        work(fake_g, groundtruth)
    except Exception:
        return False
    return True
...
def main():
    assert check(1, 1)
    assert check(P, 0)
    assert any(check(P - 1, 1) for _ in range(20))
    assert any(check(P - 1, P - 1) for _ in range(20))
